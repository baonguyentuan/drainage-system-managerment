...
const convertVn2000xyToVn2000BL = ([
  xVN2000,
  yVN2000,
  zVN2000,
]: number[] = []) => {
  ...
  let B1 =
    mu +
    ((3 * a1) / 2 - (27 * pow(a1, 3)) / 32) * sin(2 * mu) +
    ((21 * pow(a1, 2)) / 16 - (55 * pow(a1, 4)) / 32) * sin(4 * mu) +
    (151 * pow(a1, 3) * sin(6 * mu)) / 96;
...
const convertVn2000XYToWgs84XY = ([Xvn2000, Yvn2000, Zvn2000]: number[]) => {
  ...
  let Xwgs84 =
    deltaX0 + kappa * (Xvn2000 + epsilon0 * Yvn2000 - psi0 * Zvn2000);
  let Ywgs84 =
    deltaY0 + kappa * (-epsilon0 * Xvn2000 + Yvn2000 + omega0 * Zvn2000);
...
const convertWgs84XYToWgs84BL = ([Xwgs84, Ywgs84, Zwgs84]: number[]) => {
  let gamma = Math.atan(
    (Zwgs84 *
      (1 -
        f +
        (a * pow(e, 2)) /
          sqrt(pow(Xwgs84, 2) + pow(Ywgs84, 2) + pow(Zwgs84, 2)))) /
      sqrt(pow(Xwgs84, 2) + pow(Ywgs84, 2))
  );
  let Bwgs84 = Math.atan(
    (Zwgs84 * (1 - f) + a * pow(e, 2) * pow(sin(gamma), 3)) /
      ((1 - f) *
        (sqrt(pow(Xwgs84, 2) + pow(Ywgs84, 2)) -
          a * pow(e, 2) * pow(cos(gamma), 3)))
  );
  ...
  let Hwgs84 =
    cos(Bwgs84) * sqrt(pow(Xwgs84, 2) + pow(Ywgs84, 2)) +
    Zwgs84 * sin(Bwgs84) -
    a * sqrt(1 - pow(e * sin(Bwgs84), 2));
  return [(Bwgs84 * 180) / PI, (Lwgs84 * 180) / PI, 0];
};
export const convertVn2000ToWgs84 = ([
  xVN2000,
  yVN2000,
  zVN2000,
]: number[] = []) => {
