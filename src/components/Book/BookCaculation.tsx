...
const BookCaculation = (props: Props) => {
  const { structureName, lstBookCalculate, lstBookItem } = useSelector(
  ...
  const renderStation = (station: StationCalculationModel[]) => {
    return station.map((stationItem, indexStation) => {
      return (
        <div key={stationItem.idStation} className={`text-sm border-b-2`}>
            ...
            return (
              <div
                ...
                className={`grid grid-cols-7 items-center ${
                  lstAdjust.findIndex(
                    (adj) =>
                      adj.station === indexStation &&
                      adj.orientation === indexOrientation
                  ) !== -1
                    ? "bg-green-300"
                    : ""
                } `}
              >
                ...
                <div>
                  <Button
                    className={`${
                      indexOrientation === stationItem.stationStat.length - 1 ||
                      indexOrientation === 0
                        ? "hidden"
                        : "block"
                    } my-2`}
                    onClick={async () => {
                      let tempStation1 = JSON.parse(
                        JSON.stringify(stationItem)
                      );
                      tempStation1.idStation = Date.now();
                      tempStation1.stationStat.splice(
                        indexOrientation + 1,
                        tempStation1.stationStat.length - indexOrientation - 1
                      );

                      let tempStation2 = JSON.parse(
                        JSON.stringify(stationItem)
                      );
                      ...
                      await dispatch(
                        setLstBookItem({ lstBookItem: [...bookUpdate] })
                      );
                    }}
                  >
                    <BranchesOutlined />
                  ...
                  <Button
                    className={`${
                      stationItem.stationStat.length - 1 === indexOrientation ||
                      indexOrientation === 0
                        ? "block"
                        : "hidden"
                    } my-2`}
                    onClick={() => {
                      ...
                      let findIndex = lstAdjust.findIndex(
                        (adj) =>
                          adj.station === indexStation &&
                          adj.orientation === indexOrientation
                      );
  ...
  return (
    <div className="m-2">
      <h1 className="text-center text-xl">{structureName}</h1>
      <div>
      ...
      <div className="my-2">{renderStation(lstBookCalculate)}</div>
      <div className="flex justify-around items-center">
        <Button
          onClick={() => {
            let findKey = keyValue.findIndex((key) => key.name === keySelected);
            if (findKey !== -1) {
              let tempElevationPoint: OrientationPointModel = {
                name: "",
                centerNumber: 0,
                elevation: 0,
              };
                ...
                station.stationStat.forEach((orientation, orientationIndex) => {
                  if (stationIndex === 0 && orientationIndex === 0) {
                    if (orientation.note === keySelected) {
                      ...
                      newBook[stationIndex].stationStat[orientationIndex] = {
                        ...newBook[stationIndex].stationStat[orientationIndex],
                        elevation: tempElevationPoint.elevation,
                      };
                    } else {
                    ...
                    if (orientationIndex === 0) {
                      let findPoint = elevationArray.findIndex(
                        (point) => point.name === orientation.note
                      );
                      if (findPoint !== -1) {
                        tempElevationPoint = {
                        ...
                        newBook[stationIndex].stationStat[orientationIndex] = {
                          ...newBook[stationIndex].stationStat[
                            orientationIndex
                          ],
                          elevation: tempElevationPoint.elevation,
                        };
                      ...
                      let orientElevation =
                        tempElevationPoint.elevation +
                        tempElevationPoint.centerNumber -
                        orientation.centerNumber;
                      newBook[stationIndex].stationStat[orientationIndex] = {
                        ...newBook[stationIndex].stationStat[orientationIndex],
            ...
            if (lstAdjust.length >= 3) {
              lstBookCalculate.forEach((station, indexStation) => {
                station.stationStat.forEach((orientation, indexOrientation) => {
                  let compareIndex = lstAdjust.findIndex(
                    (adj) =>
                      adj.station === indexStation &&
                      adj.orientation === indexOrientation
                  );
                  if (compareIndex === -1) {
                    if (indexOrientation === station.stationStat.length - 1) {
                      tempAdj.elevation +=
                        station.stationStat[0].centerNumber -
